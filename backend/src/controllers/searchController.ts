...
export const searchArticles = async (req: Request, res: Response) => {
  try {
    ...
    // 添加筛选条件
    if (sePractice) query.sePractice = { $regex: sePractice, $options: 'i' };
    if (result) query.result = result;
    if (researchType) query.researchType = researchType;
...
export const getSearchFilters = async (req: Request, res: Response) => {
  try {
    ...
    const sePractices = await Article.distinct('sePractice', { 
      moderationStatus: 'approved', 
      analysisStatus: 'analyzed',
      sePractice: { $ne: null }
    });
    ...
    // 获取所有可用的研究类型
    const researchTypes = await Article.distinct('researchType', { 
      moderationStatus: 'approved', 
      analysisStatus: 'analyzed',
    ...
    // 获取所有可用的参与者类型
    const participantTypes = await Article.distinct('participantType', { 
      moderationStatus: 'approved', 
      analysisStatus: 'analyzed',
